...
# =========================================================
#  3. Sidebar (全項目をポップアップで統一)
# =========================================================
with st.sidebar:
    st.markdown('<h1 style="color:#e3b341; font-size:1.8rem; margin-bottom:20px;">🎼 HitForge Menu</h1>',
                unsafe_allow_html=True)

    # --- Identity 設定 ---
    with st.popover("👤 アーティスト設定", use_container_width=True):
...
st.markdown(
    '<div style="height:4px; width:200px; background:#e3b341; margin: 0 auto 30px auto; border-radius:6px;"></div>',
    unsafe_allow_html=True)
...
if st.button("⚡ GENERATE HIT-DNA PROMPT"):
    ...
    st.button("📋 Copy Style Prompt",
              on_click=lambda: st.write(f'<script>navigator.clipboard.writeText("{s_final}")</script>',
    ...
    st.button("📋 Copy Lyrics Prompt",
              on_click=lambda: st.write(f'<script>navigator.clipboard.writeText("{l_final}")</script>',
